(() => {
  ...
  let current=[],viewerPhotos=[],viewerIndex=0,returnFocus=null,heroIndex=0,heroSection=null;
  let heroPhotos=photos.filter(p=>p.own).slice(0,5);
  if(!heroPhotos.length)heroPhotos=photos.filter(p=>p.model==='모던 파운데이션').sort((a,b)=>Number(b.color.includes('아이보리'))-Number(a.color.includes('아이보리')));
  if(!heroPhotos.length)heroPhotos=photos.slice(0,4);
  ...
  const base=()=>photos.filter(p=>matchesCategory(p)&&matchesCase(p)&&matchesFrame(p)&&(!state.savedOnly||saved.has(p.id)));
  ...
  function render(){
    if(heroSection!==state.category){heroSection=state.category;heroIndex=0;
      heroPhotos=state.category==='work'?photos.filter(p=>p.own):state.category==='bed'?photos.filter(p=>p.model==='모던 파운데이션').sort((a,b)=>Number(b.color.includes('아이보리'))-Number(a.color.includes('아이보리'))):photos.filter(p=>p.section===state.category);
      heroPhotos=heroPhotos.slice(0,5);
    }
  ...
  function renderHero(){if(!heroPhotos.length){$('showcase').hidden=true;return;}const p=heroPhotos[heroIndex%heroPhotos.length];$('showcaseImage').src=p.src;$('showcaseImage').alt=p.model+' 실제 사진';$('showcaseKicker').textContent=p.own?'OUR WORK, YOUR SPACE':'IN YOUR SPACE';$('showcaseTitle').replaceChildren(document.createTextNode(p.own?'직접 손본 공간,':'우리 집에 놓이면,'),document.createElement('br'),document.createTextNode(p.own?'사진으로 남기다.':'이런 느낌.'));$('showcaseCaption').textContent=p.own?'설치와 작업 과정을 한눈에':`${p.model}${p.color?' · '+p.color:''}`;$('showcaseNote').textContent=p.own?'작업사진을 가장 먼저 만나보세요':'설치된 모습으로 살펴보세요';$('showcaseCounter').textContent=`${heroIndex+1} / ${heroPhotos.length}`;}
  ...
  function syncModal(){document.body.classList.toggle('gallery-modal-open',$('photoViewer').open||$('helpDialog').open);}
  ...
  window.addEventListener('popstate',()=>{if($('photoViewer').open)$('photoViewer').close();});
  ...
  document.addEventListener('keydown',e=>{if(!$('photoViewer').open)return;if(e.key==='ArrowRight'){e.preventDefault();nextPhoto(1);}if(e.key==='ArrowLeft'){e.preventDefault();nextPhoto(-1);}});
  ...
  $('viewerShare').addEventListener('click',async()=>{
    ...
    try{
      ...
      else{const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=file.name;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);toast('사진을 저장했어요. 카톡 등에 첨부해주세요.');}
  ...
  $('showcaseOpen').addEventListener('click',()=>{if(heroPhotos.length)openViewer(heroPhotos[heroIndex],heroPhotos);});$('showcaseNext').addEventListener('click',()=>{heroIndex=(heroIndex+1)%heroPhotos.length;renderHero();});
  renderHero();render();
})();
